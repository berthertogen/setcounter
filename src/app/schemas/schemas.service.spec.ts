...
describe('SchemasService', () => {
  test('get should load schemas from localStorage', () => {
  ...
  test('get should return empty [] when localStorage doesnt have item', () => {
    const service = new SchemasService();
  ...
  test('get(id) should return the correct schema', () => {
  ...
  test('get(id) should throw error if schema is not found', () => {
    const service = new SchemasService();
    mockLocalStorage();
  ...
  test('add should add schema to schemas in localStorage', () => {
  ...
  test('delete should remove the schema from schemas and update localStorage', () => {
    const mockSchemas = [new SchemaDefault(1), new SchemaDefault(2), new SchemaDefault(3)];
  ...
  function mockLocalStorage(schemas?: Schema[]) {
    ...
    getItem.mockReset().mockImplementation(() => (schemas ? JSON.stringify(schemas) : null));
    setItem.mockReset();
